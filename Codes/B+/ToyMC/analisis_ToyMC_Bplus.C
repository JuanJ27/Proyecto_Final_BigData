...
void analisis_ToyMC_Bplus(Int_t seed = 5, Double_t ptl=20.0, Double_t pth=23.0)
{
...
//ch->Add("ToyMC_Bc2s_1.root/tree");
 ch->Add(Form("ToyMC_Bc_%1i_%1.0f_%1.0fbest1.root/tree",seed,ptl,pth));  
TTree *tree = (TTree*) ch;
...
RooRealVar Yi("Yi"," B^{+} sig Pull",MminP,MmaxP); 
RooDataSet dataYi("dataYi","dataYi",RooArgSet(Yi));

RooRealVar Yib("Yib"," B^{+} bkg Pull",MminP,MmaxP); 
RooDataSet dataYib("dataYib","dataYib",RooArgSet(Yib)); 

RooRealVar Mu("Mu"," B^{+} mass Pull",MminP,MmaxP); 
...
canv_Yipull->Print(Form("plots/Pull_YiBs_ToyMC_%1i_%1.0f_%1.0f_best1.png",seed,ptl,pth));
...
canv_Yibpull->Print(Form("plots/Pull_Yib_ToyMC_%1i_%1.0f_%1.0f_best1.png",seed,ptl,pth));
...
canv_Mupull->Print(Form("plots/Pull_Mu_ToyMC_%1i_%1.0f_%1.0f_best1.png",seed,ptl,pth));
