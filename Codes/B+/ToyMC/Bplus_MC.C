...
void Bplus_MC(Int_t ntotal=1, Int_t seed=5, Double_t ptl=20.0, Double_t pth=23.0)
{
    /*
  ...
  */
  ...
  //*****************************************
    ...
    RooDataSet *dataToy = model.generate(RooArgSet(m), 50000);
    //RooDataSet *dataToy = model.generate(RooArgSet(m),Ns.getVal() +  Nb.getVal() );
    //RooDataSet dataToy = model.generate(RooArgSet(m), Ns.getVal() + Nb.getVal() );
    
    //cout << "---> Generé los datos" << endl;
    ...
    canv_nominal->Print(Form("plots/mass_BuFit_ptbins_ToyMC_%1.0f_%1.0f.png",ptl,pth));
    
    //canv_nominal->Print(Form("plots/mass_BuFit_ptbins_ToyMC_%1.0f_%1.0f.pdf",ptl,pth));
