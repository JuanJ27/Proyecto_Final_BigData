...
TCanvas* CreateCanvasNomPull(TString cname, RooFitResult* result, RooDataSet data, RooRealVar M, Double_t supM, Double_t infM,  RooAddPdf MassModel,RooGaussian Sig2,RooGaussian Sig, RooAddPdf sumgau, RooChebychev bkg, RooRealVar Ns, RooRealVar Nb, RooRealVar width, RooRealVar width2, RooRealVar fs, RooRealVar mean, Double_t ptl, Double_t pth,RooArgusBG argu) 
{
  ...
  MassModel.plotOn(Mframe,Components(argu),LineColor(kViolet),LineStyle(kDashed),LineWidth(2),Name("argu")); 
 ...
 leg->AddEntry(Mframe->findObject("argu")," Argus","l");
 leg->AddEntry(Mframe->findObject("bkg"),"Comb. bkg.","l");
 leg->Draw();
...
void pruebas(Double_t ptl=20.0, Double_t pth=23.0)
{
...
for(Long64_t jentry=0; jentry<nentries;jentry++)
 ...
 RooGenericPdf Sig("Sig", "Error Function", "TMath::Erf((M - mean_erf) / sigma_erf)+1", RooArgSet(M, mean_erf, sigma_erf));
 */
 ...
 RooAddPdf sumgaup("sumgaup","sumgaup",RooArgList(Sig,Sig2),RooArgList(fs));
 RooAddPdf sumgau("sumgau","sumgau",RooArgList(sumgaup,argu),RooArgList(fs));
 ...
 canv_nominal->Print(Form("plots_ptandeta1/mass_BsFit_ptbins_%1.0f_%1.0f_NOpull.png",ptl,pth));
 canv_nominal->Print(Form("plots_ptandeta1/mass_BsFit_ptbins_%1.0f_%1.0f_NOpull.pdf",ptl,pth));
 TCanvas* canv_nominalpull = CreateCanvasNomPull("canv_nominalpull", fitres, data, M, supM, infM, MassModel,Sig2 ,Sig, sumgau, bkg, Ns, Nb, width, width2, fs, mean,ptl,pth,argu);  
 canv_nominalpull->Print(Form("plots_ptandeta1/mass_BsFitpull_ptbins_%1.0f_%1.0f.png",ptl,pth));
 canv_nominalpull->Print(Form("plots_ptandeta1/mass_BsFitpull_ptbins_%1.0f_%1.0f.pdf",ptl,pth));
